...
from utils import request_game_stats
from utils import convert_time
from utils import parse_game_stats
from utils import request_game_odds
from utils import parse_odds
from utils import calculate_score_differential
from utils import get_probabilities
from utils import calculate_kelly
from utils import calculate_EV
from utils import oddsB_to_ML
from utils import get_median_EV
...
######################GAME STATS FOR EACH GAME################################
    ...
    for key in game_info_dict['games'].keys():
        game_df = game_df.append(parse_game_stats(key))
...
#######################EXTRACT WIN PROBABILITIES##############################
        ...
        for ind in final_df.index:
            ...
            lvh_prob_win, hvl_prob_win = get_probabilities(final_df, ind, oddsB_low, oddsB_high)
...
#######################APPEND EV DATA TO DATAFRAME#############################
            ...
            ev_data = [{'index': ind, 'EV_low_tier': EV_low,
                        ...
                        'lvh_prob': lvh_prob_win, 'oddsB lower tier ML':oddsB_low_ML,
                        'oddsB higher tier ML':oddsB_high_ML, 'hvl_prob':hvl_prob_win,
...
####################ISOLATE MEDIAN EV FOR EACH TIER###########################
        ...
        median_df = median_df.append(get_median_EV(EV_df_over20, 'EV_low_tier'))
        median_df = median_df.append(get_median_EV(EV_df_over20, 'EV_higher_tier'))
