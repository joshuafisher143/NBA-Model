...
def get_probabilities(df, index, oddsB_low, oddsB_high):
    ...
    return lvh_prob_win, hvl_prob_win
...
def calculate_EV(df, fractional_column, index, bet1, bank_roll, prob_win, oddsB):
    ...
    EV = ((((bet1*oddsA_low)-bet2)*.5)+(((bet2*oddsB)-bet1)*.5))*lvh_prob_win-(bet1*(1-lvh_prob_lose))
...
def get_median_EV(df, tier):
    try:
        ...
        EV_low_sort = df.iloc[(EV_df_over20[tier]-np.median(df[tier])).abs().argsort()[:2]]
